...
if(_diff != (parseNumber _val)) then
{
	...
	{
		hint format["%1 did you %2 given but you can only %3 hold so were %4 returned.",name _from,_val,_diff,((parseNumber _val) - _diff)];
	...
	else
{
	if(([true,_item,(parseNumber _val)] call life_fnc_handleInv)) then
	{
		...
		hint format["%1 did you %2 %3 given",name _from,_val,_type];
	}
		else
	{
		[[_from,_item,_val,_unit,false],"life_fnc_giveDiff",_from,false] spawn life_fnc_MP;
