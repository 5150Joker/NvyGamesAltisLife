...
while {true} do
{
	...
	{
		_countDown = if(round (_time - time) > 60) then {format["%1 Minuten",round(round(_time - time) / 60)]} else {format["%1 seconds",round(_time - time)]};
		if(isNil "life_canpay_bail") then
		{
			hintSilent format["Remaining time:\n %1\n\nYou can pay the security deposit\ndeposit: $%2",_countDown,[life_bail_amount] call life_fnc_numberText];
		}
		else
		{
			hintSilent format["Remaining time:\n %1\n",_countDown];
...
switch (true) do
{
	...
	{
		life_is_arrested = false;
		hint "You are broken out of the prison. This is an offense!";
