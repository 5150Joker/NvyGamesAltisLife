...
if((lbCurSel 38403) == -1) exitWith {hint "You have to buy or sell select an item."};
...
if((_itemInfo select 6) != "CfgVehicles") then
{
	...
	{
		if(!(player canAdd _item) && (uiNamespace getVariable["Weapon_Shop_Filter",0]) != 1) exitWith {_bad = "You do not have enough storage space for the item."};
	};
};
...
{
	hint "Selling off due SEVERE bug.";
};
	...
	else
{
	...
	hint parseText format["You buy one %1 for <t color='#8cff9b'>$%2</t>",_itemInfo select 1,[_price] call life_fnc_numberText];
	life_cash = life_cash - _price;
};
